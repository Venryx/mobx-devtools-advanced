...
function handshake(hook, contentScriptId) {
    ...
    const bridge = new Bridge_1.default({
        listen(fn) {
            const listener = evt => {
                if (evt.data.source === "mobx-devtools-content-script"
                    ...
                    debugConnection_1.default("[contentScript -> BACKEND]", evt);
                    fn(evt.data.payload);
                }
            };
        ...
        send(data) {
            debugConnection_1.default("[BACKEND -> contentScript]", data);
            window.postMessage({
                source: "mobx-devtools-backend", payload: data, contentScriptId, backendId,
            }, "*");
        },
    });
    const disposeBackend = backend_1.InitBackend(bridge, hook);
...
function waitForPing() {
    function pingListener(evt) {
        if (evt.data.source === "mobx-devtools-content-script" && evt.data.payload === "backend:ping") {
            ...
            const helloListener = e => {
                if (e.data.source === "mobx-devtools-content-script"
                    && e.data.payload === "backend:hello"
                    && e.data.contentScriptId === contentScriptId
                    && e.data.backendId === backendId) {
                    debugConnection_1.default("[contentScript -> BACKEND]", e);
    ...
    const handshakeFailedTimeout = setTimeout(() => {
        debugConnection_1.default("[BACKEND] handshake failed (timeout)");
        window.removeEventListener("message", pingListener);
    }, 10000);
