...
exports.default = onResult => {
    ...
    const getInvalidatedParentForNode = node => {
        ...
        while (current) {
            if (invalidatedNodes.has(current))
                return true;
    ...
    const getObjectForPath = path => path.reduce((acc, next) => acc && acc[next === Bridge_1.symbols.proto ? "__proto__" : next], inspectedObject);
        ...
        for (const p in node) {
            if (Object.prototype.hasOwnProperty.call(node, p)) {
                ...
                if (obj && typeof obj === "object")
                    Bridge_1.allowedComplexObjects.add(obj);
    ...
    const fireResult = (path, data) => {
        if (data && typeof data === "object") {
            Bridge_1.allowedComplexObjects.add(data);
        }
