...
function installGlobalHook(window) {
        ...
        for (let name in b)
            if (Object.prototype.hasOwnProperty.call(b, name)) {
                ...
                if (!a[name] || !b[name])
                    continue;
                for (let key in aa) {
                    if (Object.prototype.hasOwnProperty.call(aa, key) &&
    ...
    let hook = {
        ...
        onCommitFiberRoot: (rendererID, root, priorityLevel) => {
            ...
            if (officialHook)
                officialHookProps.onCommitFiberRoot(rendererID, root, priorityLevel);
        },
        onCommitFiberUnmount: (rendererID, fiber) => {
            if (officialHook)
                officialHookProps.onCommitFiberUnmount(rendererID, fiber);
        },
    };
