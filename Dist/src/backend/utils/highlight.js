...
const showNodeAroundNode = (node, targetNode, outlineColor, backgroundColor) => {
    if (!targetNode || !targetNode.offsetParent || !node)
        return;
    const offset = {
...
exports.hightlight = (node, { delay, content, borderColor, backgroundColor } = {}) => {
...
exports.stopHighlighting = node => {
    const hoverNode = hoverNodesMap.get(node);
    if (hoverNode) {
        removeHoverNode(hoverNode);
    }
};
exports.stopHighlightingAll = () => hoverNodesMap.forEach(removeHoverNode);
