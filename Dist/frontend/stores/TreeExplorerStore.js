...
const nodeMatchesText_1 = __importDefault(require("../../utils/nodeMatchesText"));
const AbstractStore_1 = __importDefault(require("./AbstractStore"));
class TreeExplorerStore extends AbstractStore_1.default {
    constructor(bridge) {
        super();
        this.roots = [];
        this.loaded = false;
        ...
        this.bridge = bridge;
        this.addDisposer(bridge.sub("frontend:mobx-react-components", components => {
        ...
        this.addDisposer(bridge.sub("frontend:mobx-react-component-updated", component => {
            this.updateNode(component);
        }));
        this.addDisposer(bridge.sub("frontend:mobx-react-component-added", component => {
            this.addNode(component);
        }));
        this.addDisposer(bridge.sub("frontend:mobx-react-component-removed", component => {
            this.removeNode(component);
        }));
        this.addDisposer(bridge.sub("picked-component", ({ componentId }) => {
            this.select(componentId, true);
            this.uncollapseParents(componentId);
            this.stopPickingComponent();
        }));
        this.addDisposer(bridge.sub("inspect-component-result", ({ componentId, path, data }) => {
        ...
        if (!text || SearchUtils.trimSearchText(text).length === 0) {
            ...
            if (this.searchRoots
                ...
                this.searchRoots = this.searchRoots.filter(item => {
                    ...
                    return ((node.name && node.name.toLowerCase().indexOf(needle) !== -1)
                ...
                this.searchRoots = Object.keys(this.nodesById).filter(key => nodeMatchesText_1.default(this.nodesById[key], needle));
            }
            ...
            data.children.forEach(childId => {
                ...
                if (!this.nodeParentsById[childId]) {
                    ...
                    if (this.searchRoots
                        ...
                        && nodeMatchesText_1.default(childNode, this.searchText.toLowerCase())) {
...
exports.TreeExplorerStore = TreeExplorerStore;
