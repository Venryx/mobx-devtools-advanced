...
describe('Components (observers) devtool', function test() {
    ...
    before(async () => {
        ...
        await driver.wait(async () => (await driver.findElements(componentsButtonLocator)).length > 0, 10000, 'Components tab wasn\'t shown');
    ...
    it('should load components tree', async () => {
        await driver.wait(async () => (await driver.findElements(componentsTreeNodeHead)).length > 0, 5000);
    });
});
