...
module.exports = async ({ initialUrl, openDevtool = true }) => {
    ...
    if (openDevtool) {
        await driver.executeScript('window.dispatchEvent(new Event(\'test-open-mobx-devtools-window\'));');
        await driver.wait(async () => (await driver.getAllWindowHandles()).length > 1, 5000, 'Devtools wasn\'t open');
        devtoolWindowHandle =
