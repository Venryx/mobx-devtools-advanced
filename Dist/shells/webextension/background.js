    ...
    waitTabLoad(+tabId, err => {
        if (err) {
            ...
            chrome.tabs.executeScript(tabId, { file: "/contentScript.js" }, res => {
                ...
                if (err || !res)
                    handleInstallError(tabId, installError);
            });
        }
    });
};
function doublePipe(one, two) {
    ...
    debugConnection_1.default(`BACKGORUND: connect ${one.name} <-> ${two.name} [${one.$i} <-> ${two.$i}]`);
    function lOne(message) {
        debugConnection_1.default(`${one.name} -> BACKGORUND -> ${two.name} [${one.$i}-${two.$i}]`, message);
        try {
    ...
    function lTwo(message) {
        debugConnection_1.default(`${two.name} -> BACKGORUND -> ${one.name} [${two.$i}-${one.$i}]`, message);
        try {
    ...
    function shutdown() {
        debugConnection_1.default(`SHUTDOWN ${one.name} <-> ${two.name} [${one.$i} <-> ${two.$i}]`);
    ...
    if (name === "content-script") {
        ...
        if (orphan) {
            ...
            port.onDisconnect.addListener(() => {
                if (__DEV__)
                    console.warn("orphan devtools disconnected"); // eslint-disable-line no-console
        ...
        if (orphan) {
            ...
            port.onDisconnect.addListener(() => {
                if (__DEV__)
                    console.warn("orphan content-script disconnected"); // eslint-disable-line no-console
                orphansByTabId[tab] = orphansByTabId[tab].filter(t => t !== newOrphan);
            });
        }
    }
