...
export default class PropVal extends React.PureComponent<Partial<{val, nested}>> {
	static propTypes = {
		val: PropTypes.any,
...
function previewProp(val, nested) {
	...
	switch (val[consts.type]) {
		...
		case "array": {
			return (
				<span className={css(styles.previewPropArray)}>
					Array[{/*val[consts.meta].length*/"TODO"}]
        </span>
			);
		}
		case "typed_array":
		...
		case "data_view": {
			return (
				<span className={css(styles.previewPropArray)}>
					{`${val[consts.name]}[${/*val[consts.meta].length*/"TODO"}]`}
				</span>
			);
		}
		case "iterator": {
